...
function FlashSale({ products }: { products: Product[] }) {
  ...
  return (
    <section className="space-y-3 px-4 py-5" aria-labelledby="flash-sale-heading">
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          <h2 id="flash-sale-heading" className="text-lg font-semibold tracking-tight">
            Flash Sale
          </h2>
        ...
        <div
          className="shrink-0 rounded-md bg-secondary px-2 py-1 text-xs font-semibold tabular-nums"
      ...
      <div className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {flashSaleProducts.map(({ card, productdetailpage }) => (
          <article
            key={card.item_group_id}
            className="w-40 shrink-0 overflow-hidden rounded-xl border bg-card text-card-foreground shadow-sm"
          >
            <div className="relative aspect-square bg-secondary">
              ...
              <span className="absolute left-2 top-2 rounded-md bg-primary px-1.5 py-1 text-[10px] font-semibold text-primary-foreground">
                Flash Sale
              </span>
            </div>

            <div className="space-y-1.5 p-3">
              <h3 className="line-clamp-2 min-h-10 text-sm font-medium">{card.title}</h3>
              ...
              <p className="truncate text-[11px] text-muted-foreground">
                {productdetailpage.availability ?? card.custom_label_0}
              </p>
            </div>
          </article>
        ))}
